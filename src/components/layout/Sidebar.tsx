...
// Menu lateral: desktop fixo; mobile em drawer com overlay, foco e bloqueio de rolagem (adendo item 7 e 13).
export function Sidebar({ atual, aberto, onFechar }:{ atual:string; aberto:boolean; onFechar:()=>void; }) {
  ...
  return (
    <>
      {aberto && <div className="fixed inset-0 bg-black/40 z-30 md:hidden" onClick={onFechar} aria-hidden />}
      <aside
        className={`bg-white border-r border-neutro-border w-64 flex-none flex flex-col z-40
          fixed inset-y-0 left-0 -translate-x-full transition-transform
          md:static md:translate-x-0 ${aberto ? "translate-x-0" : ""}`}
        aria-label="Menu principal" role="navigation"
      >
        <div className="h-[60px] flex items-center gap-2 px-4 border-b border-neutro-border">
          <Image src="/brand/logo-maracas.png" alt="Prefeitura de Maracás" width={150} height={40} priority className="h-[26px] w-auto" />
        </div>
          ...
          {ITENS.map(([key,label,ic]) => {
            ...
            return (
              <a key={key} href={`/app/${key}`} aria-current={ativo ? "page" : undefined}
                className={`relative flex items-center gap-3 h-10 px-3 my-0.5 rounded-[10px] text-sm
