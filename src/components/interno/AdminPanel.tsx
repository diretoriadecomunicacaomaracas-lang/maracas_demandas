...
export function AdminPanel({ usuarios, secretarias, unidades, graficas, grupos, internos, cargos }:
  ...
  return (
    <div>
      ...
      {tab === "Usuários" && <Usuarios usuarios={usuarios} secretarias={secretarias} unidades={unidades} graficas={graficas} cargos={cargos} run={run} pending={pending} />}
      {tab === "Secretarias" && <Secretarias itens={secretarias} run={run} pending={pending} />}
      {tab === "Setores" && <Setores unidades={unidades} secretarias={secretarias} run={run} pending={pending} />}
      {tab === "Gráficas" && <Graficas itens={graficas} run={run} pending={pending} />}
      {tab === "Grupos" && <Grupos grupos={grupos} internos={internos} run={run} pending={pending} />}
    </div>
  );
}
...
function Secretarias({ itens, run, pending }: any) {
  return <Card>
...
function Setores({ unidades, secretarias, run, pending }: any) {
...
function Graficas({ itens, run, pending }: any) {
...
function Grupos({ grupos, internos, run, pending }: any) {
...
function Usuarios({ usuarios, secretarias, unidades, graficas, cargos, run, pending }: any) {
    ...
    <div className="overflow-x-auto"><table className="w-full text-[13px] min-w-[560px]"><thead><tr className="text-left text-[11px] uppercase text-neutro-text3"><th className="py-1">Nome</th><th className="py-1">E-mail</th><th className="py-1">Ambiente</th><th className="py-1">Situação</th><th className="py-1"></th></tr></thead>
      <tbody>{usuarios.map((u: Row) => (
        <tr key={u.id} className="border-t border-neutro-border"><td className="py-1.5 font-medium">{u.nome}</td><td className="py-1.5 text-neutro-text2">{u.email}</td><td className="py-1.5 capitalize">{u.ambiente_principal}</td>
          ...
          <td className="py-1.5"><button className="text-[12px] font-semibold" onClick={() => run(A.toggleUsuario(u.id, u.situacao !== "ativa"), "Feito.")}>{u.situacao === "ativa" ? "desativar" : "ativar"}</button></td>
        </tr>))}</tbody></table></div>
